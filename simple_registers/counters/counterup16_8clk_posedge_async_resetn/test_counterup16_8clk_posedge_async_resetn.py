...
async def test_counterup16_7clk_posedge_async_resetn(dut):
  ...
  CLK7_PERIOD = 70 # [ns]
  cocotb.start_soon(Clock(dut.clock7, CLK7_PERIOD, units="ns").start()) 

  max_clock = max(CLK7_PERIOD,CLK6_PERIOD,CLK5_PERIOD,CLK4_PERIOD,CLK3_PERIOD,CLK2_PERIOD,CLK1_PERIOD,CLK0_PERIOD)
  ...
  counter_thread7 = cocotb.start_soon(counter(dut, dut.clock7, dut.cnt7_16, "count7"))  

  # Wait to finish all the threads
  await counter_thread7
