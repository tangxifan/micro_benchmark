...
def test_cocotb_rtl_list(file_db, simulator, max_num_jobs, new_thread_wait_time):
  ...
  for rtl in file_db.keys():
    rtl_abspath = os.path.abspath(curr_dir + "/" + rtl);
    logging.debug("rtl: " + rtl)
    logging.debug("rtl abspath: " + rtl_abspath)
  ...
  for rtl in file_db.keys():
    rtl_abspath = os.path.abspath(curr_dir + "/" + rtl);
    include_dir = os.path.dirname(rtl_abspath)
...
if __name__ == '__main__':
  ...
  num_errors = 0
  num_errors += test_cocotb_rtl_list(file_db, args.simulator, args.netlist_type, args.j, args.new_thread_wait_time)
